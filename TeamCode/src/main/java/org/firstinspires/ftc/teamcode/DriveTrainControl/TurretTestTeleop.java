...
@TeleOp(name = "TurretTestTeleop", group = "DriveTrainControl")
public class TurretTestTeleop extends OpMode {
    ...
    private double turretKp = 1.1;
    private double turretKi = 0.0;
    private double turretKd = 0.1;
    ...
    private static final double TURRET_SMOOTHING_ALPHA = 0.3;
    private double smoothedTurretBearing = 0.0;
    private boolean turretBearingInitialized = false;
    ...
    // === Shooter presets ===
    private final int[] rpmPresets = {2500, 2900};
    ...
    public void init() {
        ...
        telemetry.addLine("B Button: Turret Tracking");
    ...
    public void loop() {
        ...
        if (turretTrackingEnabled) {
            ...
            if (!detections.isEmpty() && detections.get(0).ftcPose != null) {
                ...
                if (!turretBearingInitialized) {
                    smoothedTurretBearing = bearing;
                    turretBearingInitialized = true;
                } else {
                    smoothedTurretBearing = TURRET_SMOOTHING_ALPHA * bearing +
                            (1.0 - TURRET_SMOOTHING_ALPHA) * smoothedTurretBearing;
                }
                ...
                double error = smoothedTurretBearing;

                turretIntegral += error * dt;
                turretIntegral = Math.max(-100.0, Math.min(100.0, turretIntegral));
                ...
                double currentServoPos = turret.getPosition();
        ...
        if (gamepad1.right_bumper && !lastRightBumper) {
            if (distanceBasedRPM) {
                ...
                if (!detections.isEmpty() && detections.get(0).ftcPose != null) {
                    ...
                    targetRPM = (distance < 110.0) ? 2500 : 2900;
                } else {
                    targetRPM = (lastValidDistance < 110.0) ? 2500 : 2850;
                }
            } else {
    ...
    private void updateTelemetry(int normPos, String shooterColorDetected) {
        ...
        // === Turret Tracking Status ===
        telemetry.addLine("=== Turret Tracking ===");
        telemetry.addData("Active", turretTrackingEnabled ? "YES (B)" : "NO");
        if (turretTrackingEnabled && hasAprilTag) {
            telemetry.addData("Servo Pos", String.format("%.3f", turret.getPosition()));
        }
        telemetry.addLine();
        ...
        telemetry.addLine("B: Turret Track");
