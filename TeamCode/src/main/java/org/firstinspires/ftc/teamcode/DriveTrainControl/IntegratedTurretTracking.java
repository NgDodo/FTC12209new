...
public class IntegratedTurretTracking extends OpMode {
    ...
    private static final int TURRET_WRAP_THRESHOLD = 100; // How close to limit before wrapping

    // === Tracking Mode ===
    private enum TrackingMode {
        OFF,           // No tracking
        BODY_TRACK,    // Rotate body to track (turret stays centered)
        TURRET_TRACK   // Rotate turret to track (body moves freely)
    }
    ...
    private static final long JAM_CHECK_INTERVAL_MS = 300;
    ...
    private static final long DETECT_TIME_MS = 100;
    ...
    public void init() {
        ...
        m2 = hardwareMap.get(DcMotorEx.class, "m2"); // Turret motor
        ...
        if (bPressed && !lastBButton) {
            switch (currentTrackingMode) {
                case OFF:
                    ...
                    resetTracking();
                    hasEverSeenTag = false; // Reset safety flag
                    break;
                case BODY_TRACK:
                ...
                case TURRET_TRACK:
                    ...
                    resetTracking();
                    hasEverSeenTag = false; // Reset safety flag
                    // Start returning turret to center
                    startTurretReturn();
                    break;
            }
        }
    ...
    private void updateTurretTracking(double robotRotation) {
        // Handle wrapping movement first
        if (turretWrapping) {
    ...
    private double getBodyTrackingCorrection() {
        ...
        if (result != null && result.isValid()) {
            ...
            if (validTarget != null) {
                ...
                bodyLastError = error;
                bodyLastTime = now;
                return -pidOut; // REVERSED: negative to flip rotation direction
            } else {
                resetTracking();
                return 0;
            }
    ...
    private boolean shouldWrapTurret(int currentPosition, double desiredOutput) {
        // Check if we're near a limit and trying to push further
    ...
    private void startTurretWrapping(int currentPosition) {
        ...
        if (currentPosition > 0) {
            // We're on the positive side, wrap to negative side
            turretWrapTarget = MIN_POSITION + TURRET_WRAP_THRESHOLD;
        } else {
            // We're on the negative side, wrap to positive side
            turretWrapTarget = MAX_POSITION - TURRET_WRAP_THRESHOLD;
        }

        // Reset PID to avoid jumps
        turretIntegral = 0;
        turretLastError = 0;
    }
    ...
    private void updateTurretWrapping() {
        ...
        // Check if we've reached the wrap target
        ...
        // Move quickly to the other side
