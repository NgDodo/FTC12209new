...
public class CloseLeftAuton extends OpMode {
    ...
    // Strafe Settings
    private static final double STRAFE_DISTANCE = 48.0; // ← Lateral distance to strafe (positive = right, negative = left)
    ...
    public void loop() {
        ...
        telemetry.addData("Position", String.format("(%.1f, %.1f)", follower.getPose().getX(), follower.getPose().getY()));
