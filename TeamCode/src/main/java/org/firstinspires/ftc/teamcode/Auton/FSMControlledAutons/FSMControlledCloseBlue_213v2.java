...
public class FSMControlledCloseBlue_213v2 extends OpMode {
    ...
    public void init() {
        ...
        // Starting pose: X = 141.5 - 123.1 = 18.4, Y unchanged, heading = 180 - 36 = 144
        follower.setStartingPose(new Pose(18.4, 123.1, Math.toRadians(144)));
    ...
    public int autonomousPathUpdateFull() {
        ...
        switch (pathState) {
            ...
            case 200: // intake over row 2, exit when (path finished + pathtimer exceeds max allowed time)
                ...
                if (follower.getPathCompletion() > 0.55 && follower.getPathCompletion() < 0.7) {
                    follower.setMaxPower(.3);
                }
            ...
            case 400:
                ...
                break;
            case 401:
                if (follower.getPathCompletion() > .9999 && (!sorter.sorterState.equals(Sorter.sorterStateFSM.SHOOTING) && !sorter.sorterState.equals(Sorter.sorterStateFSM.SHOOT_ALIGNING))) {
                    sorter.startShootingSequence(); // start shooting
                }
                if (!followerBusy) { // once we reach shooting spot
    ...
    private void buildPaths() {
        // All coordinates mirrored from Red: X = 141.5 - oldX, Y unchanged, heading = 180 - oldHeading

        Path1 = follower.pathBuilder().addPath(
                        new BezierLine(
                                new Pose(18.4,    123.100), // 141.5 - 123.1
                                new Pose(51.5,     84.000)  // 141.5 - 90.0
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(144), Math.toRadians(180)) // 180-36, 180-0
                .build();

        Path2 = follower.pathBuilder().addPath(
                        new BezierCurve(
                                new Pose(41.5,    100.000), // 141.5 - 100.0
                                new Pose(67.5,     54.976), // 141.5 - 74.0
                                new Pose(15.5,     56.000)  // 141.5 - 124.0
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180)) // 180-0, 180-0
                .build();

        Path3 = follower.pathBuilder().addPath(
                        new BezierCurve(
                                new Pose(5.5,      57.500), // 141.5 - 135.0
                                new Pose(29.5,     65.500), // 141.5 - 112.0
                                new Pose(18,     69.000)  // 141.5 - 122.0
                        )
        ...
        Path4 = follower.pathBuilder().addPath(
                        new BezierCurve(
                                new Pose(18,     69.000), // 141.5 - 122.0
                                new Pose(44.396,   60.133), // 141.5 - 97.104
                                new Pose(51.5,     84.000)  // 141.5 - 90.0
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(175), Math.toRadians(180)) // 180-5, 180-0
                .build();

        Path5 = follower.pathBuilder().addPath(
                        new BezierLine(
                                new Pose(51.5,     84.000), // 141.5 - 90.0
                                new Pose(19.5,     84.000)  // 141.5 - 122.0
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180)) // 180-0, 180-0
                .build();
        ...
        Path6 = follower.pathBuilder().addPath(
                        new BezierLine(
                                new Pose(18.5,     84.000), // 141.5 - 123.0
                                new Pose(51.5,     84.000)  // 141.5 - 90.0
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180)) // 180-0, 180-0
                .build();
        ...
        Path7 = follower.pathBuilder().addPath(
                        new BezierCurve(
                                new Pose(51.5,     84.000), // 141.5 - 90.0
                                new Pose(73.5,     34.436), // 141.5 - 68.0
                                new Pose(18.0,     33.100)  // 141.5 - 123.5
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180)) // 180-0, 180-0
                .build();
        ...
        Path8 = follower.pathBuilder().addPath(
                        new BezierLine(
                                new Pose(17.0,     33.100), // 141.5 - 124.5
                                new Pose(54.5,    115.000)  // 141.5 - 87.0
                        )
                ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(169)) // 180-0, 180-11
                .build();
    }
}